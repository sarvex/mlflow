...
class TestRunData(unittest.TestCase):
    def _check_metrics(self, metric_objs, metrics_dict, expected_metrics):
        self.assertEqual(
            {m.key for m in metric_objs}, {m.key for m in expected_metrics}
        )
        self.assertEqual(
            {m.value for m in metric_objs}, {m.value for m in expected_metrics}
        )
        self.assertEqual(
        ...
        self.assertEqual(
            {m.step for m in metric_objs}, {m.step for m in expected_metrics}
        )
